\chapter{Implementation}
    ...
    \section{Supported operations}
        ...
        There are 44 operations that FUSE exposes to filesystems to implement.
        These include the simple operations like \syscall{open},
        \syscall{read}, \syscall{write}, etc., but also advanced features like
        asynchronous operations like \syscall{poll}, extended attribute
        operations like \syscall{getxattr} or POSIX compliance features such as
        locks with \syscall{getlk}. However, none are strictly necessary and
        ...
        errors with particular functions such as \syscall{getxattr} and
        \syscall{fsync} in the case of vim. For this reason a subset of 15
        operations is implemented. These are  \syscall{lookup},
        \syscall{create}, \syscall{flush}, \syscall{release}, \syscall{access},
        \syscall{getattr}, \syscall{setattr}, \syscall{open}, \syscall{write},
        \syscall{read}, \syscall{mkdir}, \syscall{unlink}, \syscall{rmdir},
        \syscall{opendir} and \syscall{readdir}. They were chosen as a minimal
        ...
        the shell. Nice to haves like \syscall{fseek} are done away with as
    ...
    \section{B-tree}
        ...
        with making sure \syscall{malloc} and such work in that address range.
        ...
        In the end, unsurpisingly, an implementation from the Linux kernel
        (\monospace{lib/btree.c}) was chosen. It is a general purpose
